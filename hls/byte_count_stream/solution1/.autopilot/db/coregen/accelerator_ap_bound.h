...
// ==============================================================
#ifndef accelerator_ap_bound_HH_
#define accelerator_ap_bound_HH_

#include <systemc>
using namespace std;
SC_MODULE(accelerator_ap_bound) {
    static const unsigned int DATA_WIDTH = 32;
    static const unsigned int ADDR_WIDTH = 6;
    static const unsigned int accelerator_ap_bound_depth = 33;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[accelerator_ap_bound_depth];
    ...
    SC_CTOR(accelerator_ap_bound) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < accelerator_ap_bound_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (accelerator_ap_bound_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < accelerator_ap_bound_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (accelerator_ap_bound_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > accelerator_ap_bound_depth) {
...
#endif //accelerator_ap_bound_HH_
