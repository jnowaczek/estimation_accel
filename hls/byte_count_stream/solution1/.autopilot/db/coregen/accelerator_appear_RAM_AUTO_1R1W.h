...
#ifndef _accelerator_appear_RAM_AUTO_1R1W_H_
#define _accelerator_appear_RAM_AUTO_1R1W_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "accelerator_appear_RAM_AUTO_1R1W_memcore.h"

SC_MODULE(accelerator_appear_RAM_AUTO_1R1W) {
...
protected:
    ...
    // buffer instances
    accelerator_appear_RAM_AUTO_1R1W_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(accelerator_appear_RAM_AUTO_1R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "accelerator_appear_RAM_AUTO_1R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new accelerator_appear_RAM_AUTO_1R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "accelerator_appear_RAM_AUTO_1R1W.trace");
            // ports
