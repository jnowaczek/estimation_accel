...
#ifndef _byte_count_appearances_RAM_AUTO_1R1W_H_
#define _byte_count_appearances_RAM_AUTO_1R1W_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "byte_count_appearances_RAM_AUTO_1R1W_memcore.h"

SC_MODULE(byte_count_appearances_RAM_AUTO_1R1W) {
...
protected:
    ...
    // buffer instances
    byte_count_appearances_RAM_AUTO_1R1W_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(byte_count_appearances_RAM_AUTO_1R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "byte_count_appearances_RAM_AUTO_1R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new byte_count_appearances_RAM_AUTO_1R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "byte_count_appearances_RAM_AUTO_1R1W.trace");
            // ports
