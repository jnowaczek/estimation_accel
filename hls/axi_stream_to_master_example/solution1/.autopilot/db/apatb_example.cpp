...
tr.send<8>((char*)__xlx_apatb_param_outTop, 1024, 0);
}
#else
try {
static PostCheck<64> pc(AUTOTB_TVOUT_PC_gmem0);
pc.psize = 8;
pc.param = (char*)__xlx_apatb_param_outTop;
pc.depth = 1024;
...
if (__xlx_apatb_param_outTop) {
  tr.import<8>((char*)__xlx_apatb_param_outTop, 1024, 0);
}
...
unsigned char *pos = (unsigned char*)__xlx_apatb_param_outTop + i * 8;
...
{
aesl_fh.write(AUTOTB_TVIN_outTop, begin_str(AESL_transaction));
{
auto *pos = (unsigned char*)&__xlx_offset_byte_param_outTop;
aesl_fh.write(AUTOTB_TVIN_outTop, formatData(pos, 32));
}
  tcl_file.set_num(1, &tcl_file.outTop_depth);
aesl_fh.write(AUTOTB_TVIN_outTop, end_str());
}
...
for (int i = 0; i < __xlx_apatb_param_inStreamTop_stream_buf_final_size; ++i) {
  std::string s;
s = formatData((unsigned char*)(__xlx_apatb_param_inStreamTop_V_data_V_stream_buf.data()+i), 64);
aesl_fh.write(AUTOTB_TVIN_inStreamTop_V_data_V, s);
s = formatData((unsigned char*)(__xlx_apatb_param_inStreamTop_V_keep_V_stream_buf.data()+i), 8);
aesl_fh.write(AUTOTB_TVIN_inStreamTop_V_keep_V, s);
s = formatData((unsigned char*)(__xlx_apatb_param_inStreamTop_V_strb_V_stream_buf.data()+i), 8);
