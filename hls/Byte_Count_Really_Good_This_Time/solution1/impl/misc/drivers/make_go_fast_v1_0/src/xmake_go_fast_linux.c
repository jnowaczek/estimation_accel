...
typedef struct {
    ...
    XMake_go_fast_uio_map maps[ MAX_UIO_MAPS ];
} XMake_go_fast_uio_info;

/***************** Variable Definitions **************************************/
static XMake_go_fast_uio_info uio_info;
...
static int uio_info_read_name(XMake_go_fast_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMake_go_fast_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMake_go_fast_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMake_go_fast_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMake_go_fast_Initialize(XMake_go_fast *InstancePtr, const char* InstanceName) {
	XMake_go_fast_uio_info *InfoPtr = &uio_info;
